  ...
  context "Configuration Service" do 
    it "must be possible to use Configuration service retrieving all merged data for :development environnement " do 
      result_tree = {:default_value=>"value",
        :treeA=>{:trunk1=>{:branch2=>"leaf2", :branch1=>"leaf1"}, :trunk2=>{:branch5=>["value", "val1", "val2", "val3"], :branch3=>"leaf3", :branch4=>"leaf4"}},
        :setup=>
         {:rules=>
           [{:action=>:install_file, :source=>"samples/config/settings.yml", :target=>"/tmp/settings.yml"},
